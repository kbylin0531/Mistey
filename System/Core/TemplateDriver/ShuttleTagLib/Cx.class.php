...
class Cx extends TagLib {
    ...
    public function _volist($tag,$content) {
        ...
        $parseStr .= '++$'.$key.';?>';
        $parseStr .= $this->_context->parse($content);
    ...
    public function _switch($tag,$content) {
        ...
        $name       =   $this->autoBuildVar($name);
        if(count($varArray)>0)
            $name   =   $this->_context->parseTemplateFunction($name,$varArray);
    ...
    public function _case($tag,$content) {
        ...
        if('$' == substr($value,0,1)) {
            ...
            if(count($varArray)>0)
                $value  =   $this->_context->parseTemplateFunction($value,$varArray);
            $value      =   'case '.$value.': ';
        }elseif(strpos($value,'|')){
     ...
     * @return string
     */
    public function _default($tag,$content) {
