...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // session 过期
        if($_SERVER['REQUEST_TIME'] - $secode['verify_time'] > $this->config['expire']) {
            SessionUtil::set($key, null);
            return false;
        }
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->config['reset'] && SessionUtil::set($key, null);
            return true;
        }
        return false;
    }
    ...
    public function entry($id = '') {
        ...
        if($this->config['imageW']){
            $this->config['imageW'] = $this->config['length']*$this->config['fontSize']*1.5 +
                $this->config['length']*$this->config['fontSize']/2;
        }
        // 图片高(px)
        if($this->config['imageH']){
            $this->config['imageH'] = $this->config['fontSize'] * 2.5;
        }
        ...
        if($this->config['useZh']){ // 中文验证码
            for ($i = 0; $i<$this->config['length']; $i++) {
                ...
                imagettftext($this->_image, $this->config['fontSize'], mt_rand(-40, 40), $this->config['fontSize']*($i+1)*1.5, $this->config['fontSize'] + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->config['length']; $i++) {
                ...
                imagettftext($this->_image, $this->config['fontSize'], mt_rand(-40, 40), $codeNX, $this->config['fontSize']*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
        // 保存验证码
    ...
    private function _writeCurve() {
        ...
        $b = mt_rand(-$this->config['imageH']/4, $this->config['imageH']/4);   // Y轴方向偏移量
        $f = mt_rand(-$this->config['imageH']/4, $this->config['imageH']/4);   // X轴方向偏移量
        $T = mt_rand($this->config['imageH'], $this->config['imageH']*2);  // 周期
        $w = (2* M_PI)/$T;
        $px1 = 0;  // 曲线横坐标起始位置
