 ...
 */
function initDataGridEditor(selector,fields,checkLock) {
    selector = string2Jquery(selector);
    selector.datagrid({
...
//                if(lock)  return;
        ...
        onAfterEdit:function(index,dataRow){
            ...
            for(var x in fields){
                var virtualval = edit2visual(dataRow[fields[x]],scoretype);
                dataRow[fields[x]] = doWithVitualVal(virtualval)?virtualval:'';
            }
...
function getDatagridByFields(dgrid,fields){
    ...
    for(var i=0; i<rowlist.length; i++){
        ...
        for(var x in fields){
            //console.log(fields[x]);//qmcj
            var value = $.trim(rowlist[i][fields[x]]);
            console.log(value);
            //对值进行梳理
            var temp = value;
            if(arguments.length === 4){
                temp = arguments[2](value,arguments[3]);
            }else if(arguments.length === 3){
                temp = arguments[2](value);
            }
            //检查是否有错误信息
            if((temp instanceof Object) && (temp['_error'] === true)){
                //console.log(temp);
                return Messager.showMessage(temp['_info']);
            }
            result[fields[x]] = temp;//代表这个字段域 加工后的 结果
