...
//                $file_add.'-';
     ...
     * 获取文件夹下列表信息
     * 传入需要读取的文件夹路径,为程序编码
     * @param string $dir 包含结尾/   d:/wwwroot/test/
    ...
    public static function copy_dir($source, $dest){
        ...
        if ($source == substr($dest,0,strlen($source))) return false;//防止无限递归
        $result = false;
        if (is_file($source)) {
    ...
    /**
     * @param string $path 文件夹 $dir ——返回的文件夹array files ——返回的文件array
     * @param $dir
     * @param $file
